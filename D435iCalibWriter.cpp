...
void zed2(char *argv[]) {
	...
	// TODO: add option to either use zed camera or recordings
	// open the camera
	/*
	...
	camera_info = zed.getCameraInformation();
	CalibrationParameters camera_params = camera_info.camera_configuration.calibration_parameters;
	CameraParameters left_cam = camera_params.left_cam;
	CameraParameters right_cam = camera_params.right_cam;

	// tf_sl = left extrinsics from imu to camera
	Transform transform = camera_info.sensors_configuration.camera_imu_transform;
	Translation translation = transform.getTranslation();
	Rotation rotation = transform.getRotationMatrix();
	...
	Eigen::Matrix4f tf_sl = Eigen::Matrix4f::Identity();
	tf_sl.block<3, 3>(0, 0) = R_SL;
	tf_sl.block<3, 1>(0, 3) = T_SL;

	// tf_ls = left extrinsics from camera to imu
	Eigen::Matrix4f tf_ls = tf_sl.inverse();

	// tf_sr = right extrinsics from camera to imu
	Transform transform_lr = camera_params.stereo_transform;
	Transform transform_sr = transform.operator*(transform_lr);
	Translation translation_sr = transform_sr.getTranslation();
	Rotation rotation_sr = transform_sr.getRotationMatrix();
	Eigen::Vector3f T_SR(translation_sr[0], translation_sr[1], translation_sr[2]);
	Eigen::Map<Eigen::Matrix3f>R_SR(&rotation_sr.transpose(rotation_sr).r[0]);
	Eigen::Matrix4f tf_sr = Eigen::Matrix4f::Identity();
	tf_sr.block<3, 3>(0, 0) = R_SR;
	tf_sr.block<3, 1>(0, 3) = T_SR;

	// tf_rs = right extrinsics from imu to camera
	Eigen::Matrix4f tf_rs = tf_sr.inverse();
	
	// write left camera data
	ark::CameraCalibration left_calib(tf_ls.transpose(), left_cam);
	// write right camera data
	ark::CameraCalibration right_calib(tf_rs.transpose(), right_cam);

	// write camera intrinsics and extrinsics to file
	std::vector<ark::CameraCalibration> camera_calibs;
	camera_calibs.push_back(left_calib);
	camera_calibs.push_back(right_calib);
	rec.write_camera_intrinsics_and_extrinsics(camera_calibs);

	// TODO: compose and write calibration for additional cameras. currently writing left calibration file info twice.
	std::vector<ark::CameraCalibration> camera_calibs2;
	camera_calibs2.push_back(left_calib);
	camera_calibs2.push_back(left_calib);
	rec.write_additional_intrinsics_and_extrinsics(camera_calibs2);

	rec.write_camera_params();
	rec.write_imu_intrinsics();
	rec.write_additional_parameters();

	// TODO: print output from zed calibration 
...
int main(int argc, char *argv[]) try
{
	...
	if (argc == 2) {
		...
		std::cout << "usage for d435i: " << argv[0] << " <camera name>\n" 
			<< "usage for zed 2: " << argv[0] << " <camera name> <svo filepath>\n" <<
